...
public class EtherscanAPI {
    ...
    public static EtherscanAPI getInstance() {
        if (instance == null)
            Log.d("EtherscanTag", AppSettings.BASE_URL);
    ...
    public void getInternalTransactions(String address, Callback b, boolean force) throws IOException {
        if (!force && RequestCache.getInstance().contains(RequestCache.TYPE_TXS_INTERNAL, address)) {
            b.onResponse(null, new Response.Builder().code(200).message("").request(new Request.Builder()
                    .url(AppSettings.BASE_URL + "api?module=account&action=txlistinternal&address=" + address + "&startblock=0&endblock=99999999&sort=asc&apikey=" + token)
        ...
        get(AppSettings.BASE_URL + "api?module=account&action=txlistinternal&address=" + address + "&startblock=0&endblock=99999999&sort=asc&apikey=" + token, b);
    }
    ...
    public void getNormalTransactions(String address, Callback b, boolean force) throws IOException {
        if (!force && RequestCache.getInstance().contains(RequestCache.TYPE_TXS_NORMAL, address)) {
            b.onResponse(null, new Response.Builder().code(200).message("").request(new Request.Builder()
                    .url(AppSettings.BASE_URL + "api?module=account&action=txlist&address=" + address + "&startblock=0&endblock=99999999&sort=asc&apikey=" + token)
        ...
        get(AppSettings.BASE_URL + "api?module=account&action=txlist&address=" + address + "&startblock=0&endblock=99999999&sort=asc&apikey=" + token, b);
    }


    public void getEtherPrice(Callback b) throws IOException {
        get(AppSettings.BASE_URL + "api?module=stats&action=ethprice&apikey=" + token, b);
    }


    public void getGasPrice(Callback b) throws IOException {
        get(AppSettings.BASE_URL + "api?module=proxy&action=eth_gasPrice&apikey=" + token, b);
    }
    ...
    public void loadTokenIcon(final Context c, String tokenName, final boolean lastToken, final LastIconLoaded callback) throws IOException {
        ...
        get(AppSettings.BASE_URL + "/token/images/" + tokenNamef + ".PNG", new Callback() {
            @Override
    ...
    public void getGasLimitEstimate(String to, Callback b) throws IOException {
        get(AppSettings.BASE_URL + "api?module=proxy&action=eth_estimateGas&to=" + to + "&value=0xff22&gasPrice=0x051da038cc&gas=0xffffff&apikey=" + token, b);
    }


    public void getBalance(String address, Callback b) throws IOException {
        get(AppSettings.BASE_URL + "api?module=account&action=balance&address=" + address + "&apikey=" + token, b);
    }


    public void getNonceForAddress(String address, Callback b) throws IOException {
        get(AppSettings.BASE_URL + "api?module=proxy&action=eth_getTransactionCount&address=" + address + "&tag=latest&apikey=" + token, b);
    }
    ...
    public void getBalances(ArrayList<StorableWallet> addresses, Callback b) throws IOException {
        String url = AppSettings.BASE_URL + "api?module=account&action=balancemulti&address=";
    ...
    public void forwardTransaction(String raw, Callback b) throws IOException {
        get(AppSettings.BASE_URL + "api?module=proxy&action=eth_sendRawTransaction&hex=" + raw + "&apikey=" + token, b);
    }
